...
const Navbar = () => {
  ...
  return (
    <header className="w-full sticky top-0 border-b border-foreground/20 bg-background/80 backdrop-blur-sm z-50">
      <nav className="max-w-7xl mx-auto h-16 flex items-center justify-between px-6">
        <Link href="/" className="flex items-center gap-2">
          ...
          <h1 className="text-md font-semibold tracking-tight text-black dark:text-white">
            Play<span className="text-primary">PROMPT</span>
          </h1>
        </Link>
              ...
              {navlinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className={`py-2 text-sm transition-colors ${isActive ? "bg-muted text-black dark:text-white font-medium border-l-2 border-primary pl-2" : "text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white"}`}
                  >
                    {link.name}
                  </Link>
                );
